...
namespace Issue
{
    public static class Program
    {
        public static void Main(string[] args) {
			const int girls_count = 3;
			var tuples_count = IO.Read();
			
			for (var i = 0; i < tuples_count; i++) {
				var girls_candies = new int[girls_count];
				
				for (var j = 0; j < girls_count; j++)
					girls_candies[j] = IO.Read();

				var new_candies = IO.Read();

				var result = Solve(girls_candies, new_candies);

				Console.WriteLine(result);
			}
        }

		private static string Solve(int[] girls_candies, int new_candies) {
			var max = girls_candies.Maximum();
			
			var equilizer = 0;
			for (var i = 0; i < girls_candies.Length; i++) {
				equilizer += max - girls_candies[i];
			}

			var remains = new_candies - equilizer;
			
			return (remains >= 0) && (remains % girls_candies.Length) == 0
				? "YES"
				: "NO";
		}
    }
