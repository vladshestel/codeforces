    ...
    public static class Program
    {
        public static void Main(string[] args) {
			...
			for (var i = 0; i < tuples_count; i++) {
				var number = IO.Read();
				var delimeter = IO.Read();

				var result = Solve(number, delimeter);
				
				Console.WriteLine(result);
			}
        }

		private static int Solve(int number, int delimeter) {
			var remaining = number % delimeter;

			return remaining == 0 ? 0 : (delimeter - remaining);
		}
    }
