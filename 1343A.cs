    ...
    public static class Program
    {
        public static void Main(string[] args) {
			...
			for (var i = 0; i < inputs_count; i++) {
				var candies_count = IO.Read();

				var answer = Solve(candies_count);

				Console.WriteLine(answer);
			}
        }

		private static int Solve(int candies_count) {
			var k_candidate = 2; // because k > 1

			while (!IsValidCandidate(k_candidate, candies_count)) {
				k_candidate++;
			}
			
			return candies_count / (GetTwoInPowerOf(k_candidate) - 1);
		}

		private static bool IsValidCandidate(int k_candidate, int candies_count) {
			var delimeter = GetTwoInPowerOf(k_candidate) - 1; // (2 ^ k) - 1
			
			return (candies_count % delimeter) == 0;
		}

		private static int GetTwoInPowerOf(int power) {
			return (1 << power);
		}
    }
