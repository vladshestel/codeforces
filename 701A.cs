...
namespace Issue_701A
{
    internal struct Card : IComparable<Card> {
        public int Points;
        public int Number;

        public Card(int points, int number) {
            Points = points;
            Number = number;
        }

        public int CompareTo(Card other) {
            var result = this.Points - other.Points;
    ...
    public class Program
    {
        public static void Main(string[] args) {
            ...
            for (var i = 0; i < (count / 2); ++i) {
                var first = cards[i];
                var second = cards[count - i - 1];

                Console.WriteLine(string.Format("{0} {1}", first.Number, second.Number));
            }
        }
    }
