...
namespace Issue
{
    public static class Program
    {
		...
		private static int[] ReadInput() {
			const int sums_count = 4;
			
			var sums = new int[sums_count];

			for (var i = 0; i < sums_count; ++i) {
				sums[i] = IO.Read();
			}
		...
		private static void Sort(int[] numbers) {
			Sort(ref numbers[0], ref numbers[1]);
			Sort(ref numbers[0], ref numbers[2]);
			Sort(ref numbers[0], ref numbers[3]);

			Sort(ref numbers[1], ref numbers[2]);
			Sort(ref numbers[1], ref numbers[3]);

			Sort(ref numbers[2], ref numbers[3]);
		}

		private static void Sort(ref int one, ref int two) {
			if (two < one) {
		...
		private static void Solve(int[] sums) {
			var first = sums[3] - sums[2];
			var second = sums[3] - sums[1];
			var third = sums[3] - sums[0];

			var output = $"{first} {second} {third}";
