...
namespace Issue
{
    public static class Program
    {
        public static void Main(string[] args) {
			var employes_count = IO.Read();
			
			var result = Solve(employes_count);

			Console.WriteLine(result);
        }

		private static int Solve(int employes_count) {
			var combinations = 0;
			var upper_bound = employes_count / 2;

			for (var managers_count = 1; managers_count <= upper_bound; managers_count++) {
				var remaining_employers = employes_count - managers_count;

				if (remaining_employers % managers_count == 0) 
					combinations++;
			}
